# [Práctica 4: Servidor JAX-RS con Jersey](https://cvdof.ucm.es/moodle/mod/assign/view.php?id=453696)
```
Máster IoT, curso 25-26
 	└── Autores
		    ├── Ariel Gámez <arielg01@ucm.es>
 		    └── Jose Villacres <jvillacr@ucm.es>
```
**Índice**
1. [Aprobado. Servicio REST funcional con Jersey + Grizzly](https://github.com/ariegd/miot-dii/blob/LAB4/src/practica4/aprobado/README.md)
2. [Extra (notable/sobresaliente)](https://github.com/ariegd/miot-dii/blob/LAB4/src/practica4/extra/README.md)

### La aplicación deberá
```
Deberá entregar en el CV un fichero llamado practica4.zip con
├── 1) Código fuente y todos los ficheros necesarios
├── 2) Capturas o resultados
└── 3) README con integrantes del grupo, descripción, dificultades encontradas, instrucciones de compilación y ejecución
```
### Respositorio y videos de refuerzo
* [GitHub](https://github.com/ariegd/miot-dii/tree/LAB3)
* [Video browser parte 01](https://youtu.be/5HkDRjEdgxE)
* [Video browser parte 02](https://youtu.be/FyJBAlQtnPI)
* [Video android parte 01](https://youtube.com/shorts/nbkTmCIUh3I)
* [Video android parte 02](https://youtube.com/shorts/kAJvIabhJ7w) 
